        ...
        chrome.scripting.executeScript({
            ...
            if (isErrorPage) {
                chrome.storage.local.get(['refreshInterval'], (data) => {
                    ...
                    console.log(`⚠️ Server Error. Retrying in ${retryDelay / 1000}s...`);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "saveInterval" && request.refreshInterval) {
        chrome.storage.local.set(
            ...
            () => {
                console.log(`⏱️ Refresh interval set to ${request.refreshInterval}s`);
                sendResponse({ status: "ok" });
            }
        );
        return true;
    ...
    if (request.action === "clearData") {
        chrome.storage.local.remove(["rollNo", "password"], () => {
            console.log("🗑️ Credentials removed from storage.");
            sendResponse({ status: "ok" });
        });
        return true;
    }
